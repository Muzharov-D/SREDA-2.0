...
(function(){
  ...
  function ctxOf(id){
    ...
    if (id==='exec') return C(
      ...
      [['📁 Создать проект','kproj-new'],['Проекты департамента','kproj'],['Оргструктура','company']]);
    if (id==='kproj') return C(
      'Проекты департамента: кросс-отдельная работа — под каждый проект собирается команда из людей и цифровых разных направлений.',
      ['«＋ Новый проект» — опросник: тип → руководитель → состав','Клик по карточке — этапы, задачи и команда'],
      [['📁 Создать проект','kproj-new'],['Дашборд','exec']]);
    if (id==='kproj-new') return C(
      'Опросник нового проекта: как в реальном процессе — Вячеслав ведёт сам или назначает РП, состав собирается из любых направлений (есть рекомендованный по типу проекта).',
      ['Шаг 3: кнопка «✨ Рекомендованный состав» соберёт команду по шаблону'],
      [['Отменить и к списку','kproj']]);
    if (p==='kproj') return C(
      'Карточка проекта: этапы и задачи, у каждой — исполнитель и цифровой напарник. «черновик →» ведёт в петлю двойника; приёмка задачи уходит в аудит.',
      ['Чекбокс на задаче = приёмка результата','＋ строка внизу этапа добавляет задачу'],
      [['Все проекты','kproj'],['Аудит','audit']]);
    ...
    if (id==='agents') return C(
      ...
      [['Открыть спецификацию Двойник главы департамента','worker:kam-director|spec'],['Аудит','audit']]);
  ...
  const ONB = [
    ...
    { s:'team:sales', say:'Команда Продаж: 4 человека и 6 цифровых в одних функциях. У каждого цифрового — должностная инструкция с границами. Кликните «Двойник менеджера продаж» — или жмите «Далее», я открою сама.' },
    ...
    { s:'kproj', say:'Проекты департамента — здесь направления работают вместе, а не в вакууме. «＋ Новый проект» открывает опросник: вы выбираете тип, назначаете руководителя (или ведёте сами) и собираете состав из людей и цифровых любых направлений — есть рекомендованный по типу проекта.' },
  ...
  function screenTitle(){
    ...
    if (id==='kproj') return 'Проекты департамента';
    if (id==='kproj-new') return 'Новый проект · опросник';
