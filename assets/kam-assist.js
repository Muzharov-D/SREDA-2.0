...
(function(){
  ...
  function ctxOf(id){
    ...
    if (id==='mypulse') return C(
      ...
      [['Что ждёт меня?','?do'],['Открыть черновик КП','doc:kp-gamma'],['Разобрать звонок','mypulse-zoom'],['Пульс отдела','mypulse-dept']]);
    ...
    if (id==='kproj') return C(
      'Проекты департамента: кросс-отдельная работа — под каждый проект собирается команда из людей и цифровых.',
    ...
    if (p==='team'){ const d=deptOf(arg);
      return C(`Команда «${d.label}»: ${HEADCOUNT[arg]||'—'} людей + ${DIGITAL_HEADCOUNT[arg]||'—'} цифровых.`,
