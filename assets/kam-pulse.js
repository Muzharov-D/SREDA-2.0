...
(function(){
  ...
  const UDATA = {
    mgmt: {
      ...
      waiting:[
        ...
        { sev:'red',   tag:'санкция',  text:'Отправка КП банку-контрагенту — подтвердите (необратимо)', by:'platform-sales-ai' },
      ],
      drafts:[
    ...
    dev: {
      ...
      waiting:[
        ...
        { sev:'red',   tag:'санкция',  text:'Деплой ML-модели в прод — подтвердите (необратимо)', by:'agent-devops' },
    ...
    prod: {
      ...
      waiting:[
        ...
        { sev:'red',   tag:'санкция',  text:'Передача проекта в поддержку с SLA — подтвердите', by:'production-director' },
        { sev:'amber', tag:'уточнение',text:'Двойник зама: сдвигать ли веху внедрения на неделю?', open:'production-deputy' },
        { sev:'red',   tag:'приёмка',  text:'Статус-отчёт по внедрению — принять перед отправкой клиенту', open:'agent-implementation' },
      ],
      drafts:[
  ...
  const REPORTS = {
    ...
    /* подготовили материал под необратимое действие — решение остаётся за человеком */
    'platform-sales-ai': { did:['Поднял историю переговоров и профиль банка «Гамма»','Собрал КП по расширенному тарифу, сверил скидку с политикой','Согласовал юридические формулировки с двойником юриста'],
      result:'КП для банка «Гамма» готово: расширенный тариф, скидка 8% в рамках политики, срок действия 14 дней.', took:'10 минут вместо полутора часов', next:'Отправка контрагенту необратима — нужна ваша подпись' },
    'agent-devops': { did:['Собрал релизный образ ML-модели скоринга и прогнал смоук-тесты','Проверил откат: снапшот прод-версии снят, откат занимает 40 секунд','Подготовил канареечный выкат на 5% трафика'],
      result:'Модель готова к выкату: смоук-тесты зелёные, откат проверен, канарейка на 5% трафика на 2 часа.', took:'25 минут', next:'Деплой в прод необратим — нужна ваша подпись' },
    'agent-implementation': { did:['Свёл прогресс по вехам внедрения за неделю','Собрал замечания клиента и статус их устранения','Подготовил статус-отчёт с рисками и следующими шагами'],
      result:'Статус-отчёт готов: развёртывание завершено, первая группа обучена, веха «Сдача» под риском сдвига на неделю.', took:'15 минут', next:'После вашей приёмки уходит клиенту' },
  };
  ...
  /* чего именно ждёт цифровой сотрудник — из тега его точки участия */
  const PILL_BY_TAG = { 'приёмка':'ждёт вашей приёмки', 'уточнение':'ждёт вашего ответа', 'санкция':'ждёт вашей подписи', 'решение':'ждёт вашего решения' };
  const staffPill = (w)=>{
    const it = waitingItems().find(x=>x.open===w.id || x.by===w.id);
    if (!it) return `<span class="mp-pill" style="${GREEN_PILL}">в работе</span>`;
    return `<span class="mp-pill ${it.sev}">${PILL_BY_TAG[it.tag]||'ждёт вас'}</span>`;
  };
